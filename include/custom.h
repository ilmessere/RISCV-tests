...
volatile static inline void custom0_rd_rs1_rs2(const unsigned x, unsigned res, unsigned op1, unsigned op2){
    asm volatile("custom0_RD_RS1_RS2_%c3 %0, %1, %2"	    
		    :                       	                  
		    : "r"(res), "r"(op1), "r"(op2), "i"(x));
}

volatile static inline void custom0_rd_rs1(const unsigned x, unsigned res, unsigned op1, const unsigned rs2){
    asm volatile("custom0_RD_RS1_%c[funct] %[rd], %[rs1], x%c[rs2]"	    
		    :                      	                  
		    : [rd]"r"(res), [rs1]"r"(op1), [rs2]"i"(rs2), [funct]"i"(x));
}
